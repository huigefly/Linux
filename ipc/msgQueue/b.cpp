...
void *onProcSend(void *lParam)
{
	...
	while (1)
    {
    	char str1[] = "BBBB test message: Wellcome.";
    	MSG_BUF_T sndmsg;
    	memset (&sndmsg, 0, sizeof(sndmsg));
    	
    	sndmsg.msgtype = 100;  
	    sprintf(sndmsg.msgtext, "%s", str1);  
	    if(msgsnd(msgid, (struct msgbuf *)&sndmsg, sizeof(sndmsg.msgtext), IPC_NOWAIT) == -1)  
	    {
	    	printf ("msgsnd error\n\n");
	    	// break;
	    }
...
void *onProcRecv(void *lParam)
{
	...
	char str2[] = "BBBB test message: goodbye.";  
    MSG_BUF_T rcvmsg;
	while (1)
	{
    	memset (&rcvmsg, 0, sizeof(rcvmsg));
		if(msgrcv(msgid, (struct msgbuf*)&rcvmsg, sizeof(rcvmsg), 200, MSG_NOERROR) == -1)  
	    {
	    	printf ("msgrcv error\n");
	    	//pthread_exit((void *)0);
	    	sleep(5);
